...
数据库连接信息保存在 Tauri 应用数据目录下的 `connections.json` 文件中，具体位置由系统和 Tauri 的 `app_data_dir` 决定。保存连接时，密码会以 Base64 编码写入 `passwordEncrypted` 字段；Base64 只是编码，不具备加密保护能力。请不要将包含真实数据库凭据的 `connections.json` 提交到仓库或对外共享。当前 SSL 选项用于启用加密连接，但代码会跳过证书域名校验并接受无效证书，因此不等同于严格的生产级证书校验。